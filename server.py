...
def scraper(start_dir, end_dir):
    ...
    def miniworker(threads, direccion):
        ...
        try:
            ...
            if (cantidad != -1 and cantidad < 4 and (direccion[:2] == "MZ" or direccion[:2] == "RS")) or (cantidad == 0 and direccion[:2] == "RK"):
...
@socketio.on('TL-data')
def TL_data(text):
    # Funcion que recibe el sector (MZ, RK o RS) y las posiciones, o sea piso o nivel (MZ-0, RK-2, RS-0)
    # y devuelve la un array con las cantidades totales de ocupacion/libre
    def foo(sector, posiciones):
        total = {}
        for pos in posiciones:
            total[pos] = 0
        for pos in posiciones:
            ...
            sector[0][pos] = num
        sector.append(total)
        # Obtengo las posiciones ocupadas por piso [SECTOR]
        sector.append({})
        for pos in posiciones:
            total[pos] = 0
        for pos in posiciones:
