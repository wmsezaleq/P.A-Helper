...
def scraper(start_dir, end_dir):
    ...
    def worker(row):
        try:
            if row[0][:4] in ("NA-9", "NI-9"):
                row[0] = "RK-0" + row[0][4:]
            elif row[0][:2] == "HV" or row[0][:2] == "NA" or row[0][:2] == "NI" or row[0][:2] == "DI" or row[0][:2] == "DM" or row[0] or row[0][:2] == "RS":
                row[0] = "MZ" + row[0][2:]
    ...
    def miniworker(threads, direccion):
        ...
        try:
            ...
            Metrica.add([direccion, Metrica.get_free_percentage(volumen, direccion[:2]), cantidad])
            ...
            foo(MZ2, ("RS-0", "MZ2-1", "MZ2-2", "MZ2-3"))
            ...
            print("POSIBLE ERROR DE CONEXION... REINICIE EL SERVIDOR: {}".format(traceback.format_exc()))
    ...
    def foo():
        ...
        if sec == "MZ":
            ...
            if (piso == 0) and (calle == 26):
                ...
                start_dir = f"RS-0-{calle}-001-01-01"
                end_dir = f"RS-0-{calle}-086-01-01"
                scraper(start_dir, end_dir)
...
def update_floor(msg):
    def t():
        for calle in range(27):
            x = threading.Thread(target=buscarLugar, daemon=True, args=("MZ", int(msg), calle+1))
            x.start()
            time.sleep(20)
